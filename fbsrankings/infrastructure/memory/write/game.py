...
class GameRepository (BaseRepository):
    ...
    def find_by_ID(self, ID):
        return self._to_game(self._storage.find_by_ID(ID))
    ...
    def find_by_season_teams(self, season, week, team1, team2):
        ...
        return self._to_game(self._storage.find_by_season_teams(season_ID, week, team1_ID, team2_ID))
    ...
    def find_by_season(self, season):
        ...
        return [self._to_game(item) for item in self._storage.find_by_season(season_ID)]
        
    def all(self):
        return [self._to_game(item) for item in self._storage.all()]
    ...
    def _handle_game_rescheduled(self, event):
        dto = self._storage.find_by_ID(event.ID)
        dto.week = event.week
        dto.date = event.date
