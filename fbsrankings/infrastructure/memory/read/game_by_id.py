...
class GameByIDQueryHandler (object):
    ...
    def handle(self, query):
        game = self._storage.game.find_by_ID(query.ID)
        if game is not None:
            season = self._storage.season.find_by_ID(game.season_ID)
            home_team = self._storage.team.find_by_ID(game.home_team_ID)
            away_team = self._storage.team.find_by_ID(game.away_team_ID)
