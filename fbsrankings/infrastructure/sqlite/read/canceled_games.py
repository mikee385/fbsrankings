...
class CanceledGamesQueryHandler (object):
    ...
    def handle(self, query):
        ...
        items = [CanceledGameResult(UUID(row[0]), UUID(row[1]), row[2], row[3], row[4], SeasonSection[row[5]], UUID(row[6]), row[7], UUID(row[8]), row[9], row[10]) for row in cursor.fetchall()]
        cursor.close()
