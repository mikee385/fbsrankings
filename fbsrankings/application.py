    ...
    def ImportTeamsFromCsv(self, year, csv_reader):
        ...
        for row in iterrows:
            if row[rank_index].isdigit():
                ...
                self._repository.AddAffiliation(season, team, Subdivision.FBS)
    ...
    def ImportGamesFromCsv(self, year, postseason_start_week, csv_reader):
        ...
        for counter, row in enumerate(csv_reader):
            if row[rank_index].isdigit():
                ...
                home_team = self._repository.FindTeamByName(home_team_name)
                if home_team is None:
                    home_team = self._repository.AddTeam(home_team_name)
                    
                home_team_affiliation = self._repository.FindAffiliationBySeasonTeam(season, home_team)
                if home_team_affiliation is None:
                    self._repository.AddAffiliation(season, home_team, Subdivision.FCS)
                
                away_team = self._repository.FindTeamByName(away_team_name)
                if away_team is None:
                    away_team = self._repository.AddTeam(away_team_name)
                    
                away_team_affiliation = self._repository.FindAffiliationBySeasonTeam(season, away_team)
                if away_team_affiliation is None:
                    self._repository.AddAffiliation(season, away_team, Subdivision.FCS)
